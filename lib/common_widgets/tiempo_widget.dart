      ...
      return Column(
        ...
        children: [
          ...
          CustomTile(
              ...
              subText:
              DateFormat('HH:mm:ss').format(viajesModel.unloadingTimeInIndustry)),
          ...
          Consumer(
            builder: (BuildContext context, WidgetRef ref, Widget? child) {
              return ref.watch(getAllViajesForIndustryList(IndustryAndVesselIdsModel(industryId: viajesModel.industryId, vesselId:viajesModel.vesselId))).
              when(
                data: (viajesList){
                  if(viajesList.isEmpty){
                    return SizedBox();
                  }
                  Duration avgTimeToIndustryFromPort=calculateAvgArrivalTime(viajesList);
                  Duration avgTimeUnloading=calculateAvgUnloadingTime(viajesList);
                  Duration delayTimeToIndustryFromPort=(avgTimeToIndustryFromPort-(viajesModel.timeToIndustry
                      .difference(viajesModel.exitTimeToPort)));
                  Duration delayTimeUnloading=(avgTimeUnloading-(viajesModel.unloadingTimeInIndustry
                      .difference(viajesModel.timeToIndustry)));
                  ...
                  return  Column(
                    children: [
                      ...
                      CustomTile(title: "Tiempo promedio a destino", subText: formatDuration(avgTimeToIndustryFromPort)),

                      //Average time of unloading (sum of "tiempo de descarga" for all the "viajes" for that industry/ qty of "viajes"
                      CustomTile(title: "Tiempo promedio de descarga", subText: formatDuration(avgTimeUnloading)),

                      //Delayed time (Tiempo promedio a destino - Tiempo a destino)
                      CustomTile(
                        title: "Tiempo de retraso a destino",
                        subText: formatDuration(delayTimeToIndustryFromPort.abs()),
